...
ggplot(data=mdcovid_bardata, aes(x=date, y=dailycase))+
  geom_bar(stat="identity", width= 0.8) + scale_x_discrete(breaks = mdcovid_bardata$date[c(T,F,F,F,F,F,F,F)]) +
...
mdplot<- ggplot(data=mdcovid_bardata, aes(x=date, group=1 ))+
  ...
  geom_line(aes(y = average7), stat="identity", color="red", size=1.2) +
  scale_x_discrete(breaks = mdcovid_bardata["date"][seq(1, nrow(mdcovid_bardata), 10),])

mdplot<- ggplot(data=mdcovid_bardata, aes(x=date, group=1 ))+
  geom_bar(aes(y=dailycase), stat="identity", width= 0.5, color="blue", fill="blue") + 
  geom_line(aes(y = average7), stat="identity", color="red", size=1.2) +
  scale_x_discrete(breaks = mdcovid_bardata["date"][rev(seq(nrow(mdcovid_bardata), 1, -15)),]) 
